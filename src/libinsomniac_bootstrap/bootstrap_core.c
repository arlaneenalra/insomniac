...
gc_type_def register_node_double_type(gc_type *gc) {
    ...
    gc_register_pointer(
        ...
        offsetof(ins_node_type, value) + offsetof(node_value_type, two) +
            offsetof(two_stream_type, stream1));
    gc_register_pointer(
        gc, type,
        offsetof(ins_node_type, value) + offsetof(node_value_type, two) +
...
void setup_include(compiler_core_type *compiler, ins_stream_type *arg) {
    ...
    /* Unless the file starts with a / look for it in the current include
     * directory */
    if (file_name[0] == '/' || compiler->include_depth < 0) {
        ...
        if (length > PATH_MAX) {
            (void)fprintf(
                stderr,
                "Error %i! Including '%s' - Search path too long: %zi "
                "characters\n",
...
void compile_file(compiler_type *comp_void, char *file_name, bool include_baselib) {
    ...
    if (!in) {
        (void)fprintf(
            stderr, "Error %i while attempting to open '%s'\n", errno, file_name);
        assert(0);
    }
...
void compiler_code_gen(compiler_type *comp_void, buffer_type *buf, bool bootstrap) {
