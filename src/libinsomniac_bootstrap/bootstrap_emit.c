...
void emit_cond(
    compiler_core_type *compiler, buffer_type *buf, ins_node_type *node,
...
void emit_lambda(compiler_core_type *compiler, buffer_type *buf, ins_node_type *node) {
    ...
    while (formal_node && formal_node->type != STREAM_LITERAL) {
        ...
        /* If we see the end of the list here, we're binding anything left in
           the arguments list to this symbol. */
...
bool emit_node(
    ...
    switch (head->type) {
        ...
        case STREAM_MATH:
            emit_double(
                compiler, buf, head->value.two.stream2->head,
