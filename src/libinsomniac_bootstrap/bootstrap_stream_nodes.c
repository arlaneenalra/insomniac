...
BUILD_SINGLE_STREAM(record_type, STREAM_RECORD_TYPE)

/* Double stream nodes */
BUILD_DOUBLE_STREAM(bind, STREAM_BIND)
BUILD_DOUBLE_STREAM(store, STREAM_STORE)

BUILD_DOUBLE_STREAM(let_star, STREAM_LET_STAR)
...
void stream_bare(
    compiler_core_type *compiler, ins_stream_type *stream, node_type type, char *lit) {
