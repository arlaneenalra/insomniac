...
void gc_register_pointer(gc_type *gc_void, gc_type_def type, size_t offset) {
    gc_ms_type *gc = (gc_ms_type *)gc_void;
    meta_obj_def_type *meta_type = MALLOC_TYPE(meta_obj_def_type);


    meta_type->next = gc->type_defs[type];
    meta_type->offset = offset;
    gc->type_defs[type] = meta_type;
...
void destroy_types(meta_obj_def_type **type_list, uint32_t num_types) {
    meta_obj_def_type *next=0;
    meta_obj_def_type *type_def=0;
    

    for(int i=0; i<num_types; i++) {
        type_def=type_list[i];
